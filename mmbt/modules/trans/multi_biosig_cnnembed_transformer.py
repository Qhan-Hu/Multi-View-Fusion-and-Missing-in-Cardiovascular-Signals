...
class BioSigTransformer(nn.Module):
    def __init__(
        ...
        self.ppg_patch_embed = resnet18(config["hidden_size"], in_chan=3)
        self.ecg_patch_embed = resnet18(config["hidden_size"], in_chan=1)
        ...
        if config["pos_embed"] == "CS":
            self.ecg_pos_embed = CosinePositionEncoding(embed_dim=embed_dim, seq_len=self.num_patches+1)
            self.ppg_pos_embed = CosinePositionEncoding(embed_dim=embed_dim, seq_len=self.num_patches+1)
        if config["pos_embed"] == "LP":
            self.ecg_pos_embed = LearnablePositionalEncoding(embed_dim=embed_dim, seq_len=self.num_patches+1)
            self.ppg_pos_embed = LearnablePositionalEncoding(embed_dim=embed_dim, seq_len=self.num_patches+1)
        if config["pos_embed"] == "":
    ...
    def multi_signal_embed(self, _ppg, _ecg):
        ...
        B, _, _ = _ppg.shape
        _ppg = _ppg[:, :, :self.num_patches * self.patch_size]
        _ecg = _ecg[:, :, :self.num_patches * self.patch_size]

        # (B, C, L) -> (B, C, num_patches, patch_size) -> (B, num_patches, C, patch_size) -> (B*num_patches, C, patch_size)
        _ppg = _ppg.reshape(B, 3, self.num_patches, self.patch_size).permute(0, 2, 1, 3).reshape(-1, 3, self.patch_size)
        _ecg = _ecg.reshape(B, 1, self.num_patches, self.patch_size).permute(0, 2, 1, 3).reshape(-1, 1, self.patch_size)

        ppg_embed = self.ppg_patch_embed(_ppg).reshape(B, self.num_patches, -1)
        ecg_embed = self.ecg_patch_embed(_ecg).reshape(B, self.num_patches, -1)
        ...
        if self.add_norm_before_transformer:
            ppg_embed = self.pre_norm(ppg_embed)
            ecg_embed = self.pre_norm(ecg_embed)
